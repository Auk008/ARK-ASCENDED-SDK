...
public:
	...
	void FindAssetNativeClass(const struct FAssetData& AssetData, class UClass* ReturnValue);
...
public:
	...
	void UseFilterToExcludeAssets(const TArray<struct FAssetData>& AssetDataList, const struct FARFilter& Filter);
	...
	void RunAssetsThroughFilter(const TArray<struct FAssetData>& AssetDataList, const struct FARFilter& Filter);
	...
	void K2_GetAssetByObjectPath(const struct FSoftObjectPath& ObjectPath, bool bIncludeOnlyOnDiskAssets, const struct FAssetData& ReturnValue);
	...
	void GetAssetsByPaths(const TArray<class FName>& PackagePaths, const TArray<struct FAssetData>& OutAssetData, bool bRecursive, bool bIncludeOnlyOnDiskAssets, bool ReturnValue);
	void GetAssetsByPath(class FName PackagePath, const TArray<struct FAssetData>& OutAssetData, bool bRecursive, bool bIncludeOnlyOnDiskAssets, bool ReturnValue);
	...
	void GetAssets(const struct FARFilter& Filter, const TArray<struct FAssetData>& OutAssetData, bool bSkipARFilteredAssets, bool ReturnValue);
...
{
public:
	uint8                                        Pad_2F43[0xB90];                                   // Fixing Size Of Struct [ Dumper-7 ]
